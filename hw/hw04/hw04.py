def shuffle(s):
    ...
    for i in range(len(s)//2):
        res_list[2 * i] = s[i]
        res_list[2 * i + 1] = s[i + len(s) // 2]
    return res_list
    """
    n = len(s) // 2
    if n == 0: return []
    else:
        return [s[0], s[n]] + shuffle(list(s[1:n]) + list(s[n + 1:]))
    # 优秀的递归解法
...
def deep_map(f, s):
    ...
    for i in range(len(s)):
        if type(s[i]) != list:
            ...
            deep_map(f, s[i])
    #要求在屏幕上没有输出，所以反而不能return。要遍历索引而不能遍历元素
...
def balanced(m):
    ...
    torque_left = length(left_arm) * total_mass(left_end)
    torque_right = length(right_arm) * total_mass(right_end)
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for branch_t in list(branches(t)):
            if berry_finder(branch_t):
                return True
        return False
    # 不要太早return，否则每个树只有第一个分支会被检查
...
def max_path_sum(t):
    ...
    if is_leaf(t):
        ...
        max_sum = 0
        for branch_t in list(branches(t)):
            if max_path_sum(branch_t) > max_sum:
                max_sum = max_path_sum(branch_t)
        return label(t) + max_sum
    # 递归思想。若为leaf则直接返回值，若为tree则对branches进行递归，直至递归到leaf
