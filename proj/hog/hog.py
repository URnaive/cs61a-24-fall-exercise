...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    total = 0
    outcome_list = [dice() for _ in range(num_rolls)]
    for i in outcome_list:
        if i == 1:
            return 1
    return sum(outcome_list)
    # END PROBLEM 1
"""将所有掷色子结果先存储在一个列表内，再逐一读取并验证，最终返回结果"""
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    score = max(3 * abs(((opponent_score // 10) % 10) - (player_score % 10)), 1)
    return score
    # END PROBLEM 2
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    last_player_score, last_opponent_score = player_score, opponent_score
    if num_rolls == 0:
        return boar_brawl(player_score, opponent_score)
    else:
        return roll_dice(num_rolls, dice)
    # END PROBLEM 3
"""当玩家选择掷色子0次（即跳过本次掷色子环节）时，调用boar_brawl函数计算玩家加分；当玩家掷色子1至10次时，调用roll_dice函数计算得分。"""
...
def sus_points(score):
    ...
    if num_factors(score) == 3 or num_factors(score) == 4:
        for new_score in range(score + 1, 2 * score + 1):
            if is_prime(new_score) == True:
                return new_score
    return score
    # END PROBLEM 4
...
"""注意是每个玩家轮流走一步，用变量who决定哪位玩家的回合，不要写成每次连续走两步！"""
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def func(player_score, opponent_score):
        return n
    return func
    # END PROBLEM 6
"""直接输出n即可"""
