...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    word_list = [word for word in paragraphs if select(word) == True]
    if k >= 0 and k <= len(word_list) - 1:
        return word_list[k]
    else:
        return ''
    # END PROBLEM 1
...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def search_about_word(sentence):
        target_list = split(lower(remove_punctuation(sentence)))
        for about_word in subject:
            if about_word in target_list:
                return True
        return False
    return search_about_word
    # END PROBLEM 2
"""调用utils.py中已定义的函数，以此减轻工作量"""
